  ...
  defmodule Nomad.AWS.VirtualMachines do
    ...
    def get_virtual_machine(region, instance, fun \\ &list_virtual_machines/1) do
      ...
      cond do
        is_list(res) ->
          res
          |> Enum.filter(fn {_a, b, c, d} = vm -> vm == {instance, b, c, d} end)
          |> List.first
        true -> res
      end
    end

    def get_virtual_machine!(region, _instance, fun \\ &list_virtual_machines/1), do: fun.(ExAws.EC2.new(region: region))
    ...
    def get_disk(region, disk, fun \\ &list_disks/1) do
      ...
      cond do
        is_list(res) ->
          [vol] = res
          |> Enum.filter(fn {a, _b, _c, _d, _e} -> a == disk end)
    ...
    def get_disk!(region, _disk, fun \\ &list_disks/1) do
      cond do
        Mix.env() == :text -> fun
        true               -> fun.(region)
      end
