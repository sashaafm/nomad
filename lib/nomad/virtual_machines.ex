defmodule Nomad.VirtualMachines do
  ...
  @adapter Application.get_env(:nomad, :vm_module)

  defdelegate list_virtual_machines(region),                              to: @adapter
  defdelegate list_virtual_machines!(region),                             to: @adapter
  defdelegate get_virtual_machine(region, instance),                      to: @adapter
  defdelegate get_virtual_machine!(region, instance),                     to: @adapter
  defdelegate create_virtual_machine(region, class, image, auto_delete),  to: @adapter
  defdelegate create_virtual_machine!(region, class, image, auto_delete), to: @adapter
  defdelegate delete_virtual_machine(region, instance),                   to: @adapter
  defdelegate delete_virtual_machine!(region, instance),                  to: @adapter
  defdelegate start_virtual_machine(region, instance),                    to: @adapter
  defdelegate start_virtual_machine!(region, instance),                   to: @adapter
  defdelegate stop_virtual_machine(region, instance),                     to: @adapter
  defdelegate stop_virtual_machine!(region, instance),                    to: @adapter
  defdelegate reboot_virtual_machine(region, instance),                   to: @adapter
  defdelegate reboot_virtual_machine!(region, instance),                  to: @adapter
  defdelegate set_virtual_machine_class(region, instance, class),         to: @adapter
  defdelegate set_virtual_machine_class!(region, instance, class),        to: @adapter
  defdelegate list_disks(region),                                         to: @adapter
  defdelegate list_disks!(region),                                        to: @adapter
  defdelegate get_disk(region, disk),                                     to: @adapter
  defdelegate get_disk!(region, disk),                                    to: @adapter
  defdelegate create_disk(region, size),                                  to: @adapter
  defdelegate create_disk!(region, size),                                 to: @adapter
  defdelegate create_disk(region, size, image),                           to: @adapter
  defdelegate create_disk!(region, size, image),                          to: @adapter
  defdelegate delete_disk(region, disk),                                  to: @adapter
  defdelegate delete_disk!(region, disk),                                 to: @adapter
  defdelegate attach_disk(region, instance, disk, device_name),           to: @adapter
  defdelegate attach_disk!(region, instance, disk, device_name),          to: @adapter
  defdelegate detach_disk(region, instance, disk),                        to: @adapter
  defdelegate detach_disk!(region, instance, disk),                       to: @adapter
  defdelegate list_regions(),                                             to: @adapter
  defdelegate list_regions!(),                                            to: @adapter
